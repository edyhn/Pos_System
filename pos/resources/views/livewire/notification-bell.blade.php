<div wire:poll.30s="refreshCount" class="relative" x-data="{ open: @entangle('showDropdown') }">
    <button wire:click="toggle" class="relative p-2 text-gray-600 hover:text-gray-900 transition">
    ...
    @if($showDropdown)
        <div class="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-lg border border-gray-200 z-50 max-h-96 overflow-y-auto" @click.away="open = false">
            ...
            @else
                ...
                @foreach($lowStockProducts as $product)
                    <a href="{{ route('stock.index') }}" class="flex items-start gap-3 p-3 hover:bg-gray-50 transition border-b border-gray-50">
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="text-sm text-gray-800 truncate">Stok <strong>{{ $product->name }}</strong> menipis</p>
                            <p class="text-xs text-gray-500">{{ $product->stock }} / {{ $product->min_stock }}</p>
                        </div>
                    </a>
                @endforeach
            ...
            <div class="p-2 border-t border-gray-100 text-center">
                <button wire:click="markAllRead" class="text-xs text-blue-600 hover:underline">Tandai sudah dibaca</button>
            </div>
        </div>
    @endif
</div>
